<?php
namespace ColourStream\CronBundle\Repostory;
...
class CronJobResultRepository extends EntityRepository
{
    public function deleteOldLogs(CronJob $job = null)
    {
        ...
        $data = $this->getEntityManager()
            ->createQuery(
                "SELECT job.id, MAX(result.id) FROM ColourStreamCronBundle:CronJob job
        ...
        foreach($data as $datum)
        {
            ...
            if(!$job || $job->getId() == $jobId) {
                $this->getEntityManager()->createQuery(
                    "DELETE ColourStreamCronBundle:CronJobResult result
